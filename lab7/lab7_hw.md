...
Some of the NA's in amniota_tidy are represented by "-999" instead of NA, so the number of NA's in `amniota` dataset makes no sense.
...
```
Yes. Since most of mammals do not lay eggs, the percentage of NA's in egg_mass_g column should be near 100%. This implies that not all NA's are due to missing data or failure to record.
...
## 4 Ter        1104     16.3
```
These NA's are probably due to loss/failure of record.
